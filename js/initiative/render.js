...
function renderTimeline(data){
  ...
  var rowsHtml=tlData.map(function(d){
    ...
    var missingGl=(isDone&&!glDate)?'<span style="font-size:9px;color:var(--down);font-weight:600;margin-left:6px">⚠ Missing Go-live Date</span>':'';
    var glLabel=(glDate?'<span style="font-size:9px;color:var(--purple);font-weight:600;margin-left:6px">◆ Go-live: '+fmtFullDate(glDate)+'</span>':missingGl)+doneTag;
...
function _buildGroupedCompDropdown(availComps, activeArr){
  ...
  listEl.onclick=function(e){
    ...
    if(e.target.closest('.gdb-grp-clear')){ sumComponentFilter.splice(0); renderSummary(); var p=document.getElementById('comp-dropdown-panel');if(p)p.style.display='block'; return; }

/* ── Support Render Patch — overlay buildGroupChart + buildHeatmapAC ──
   Load this AFTER js/support/render.js in index.html
── */

/* Override buildGroupChart: bar chart → pie chart */
function buildGroupChart(data) {
  ...
  var COLORS = ['#378ADD','#3B6D11','#EF9F27','#E24B4A','#534AB7','#1D9E75'];
  var W = Math.max(el.offsetWidth || 0, 220);
  var R = Math.min(W * 0.38, 72);
  var cx = W / 2, cy = R + 12;
  var H = R * 2 + 24 + Math.ceil(entries.length / 2) * 18;
  ...
  entries.forEach(function(e, i) {
    ...
    svg += '<path d="M' + cx + ' ' + cy + ' L' + x1 + ' ' + y1 + ' A' + R + ' ' + R + ' 0 ' + large + ' 1 ' + x2 + ' ' + y2 + ' Z"'
         + ' fill="' + col + '" stroke="#1c2128" stroke-width="2">'
         + '<title>' + e[0] + ': ' + e[1] + ' (' + Math.round(e[1] / total * 100) + '%)</title></path>';
    if (sweep > 0.2) {
      var mx = cx + R * 0.6 * Math.cos(startAngle + sweep / 2);
      var my = cy + R * 0.6 * Math.sin(startAngle + sweep / 2);
      svg += '<text x="' + mx + '" y="' + my + '" text-anchor="middle" dominant-baseline="middle"'
           + ' font-size="10" font-weight="600" fill="#fff">' + Math.round(e[1] / total * 100) + '%</text>';
    }
    startAngle += sweep;
  });
  var legY = R * 2 + 24;
  entries.forEach(function(e, i) {
    ...
    var lx = 8 + col * (W / 2), ly = legY + row * 18;
    var label = e[0].length > 20 ? e[0].substring(0, 19) + '\u2026' : e[0];
    svg += '<rect x="' + lx + '" y="' + ly + '" width="9" height="9" rx="2" fill="' + COLORS[i % COLORS.length] + '"/>';
    svg += '<text x="' + (lx + 12) + '" y="' + (ly + 8) + '" font-size="9.5" fill="#8b949e">'
         + label + ' (' + e[1] + ')</text>';
  });
  svg += '</svg>';
  el.innerHTML = svg;
...
/* New: buildHeatmapAC — Volume (A) + Avg Effort (C) heatmaps */
function buildHeatmapAC(data) {
  ...
  groups.sort();
  var maxV = months.reduce(function(m, mo) {
    return Math.max(m, groups.reduce(function(s, g) {
      return Math.max(s, (vol[mo] && vol[mo][g]) ? vol[mo][g] : 0);
    }, 0));
  }, 0) || 1;
  ...
  function bt(elId, cfn, obj) {
    ...
    months.forEach(function(mo) {
      var p = mo.split('-'), ml = MS[parseInt(p[1]) - 1] + ' ' + p[0].slice(2);
      ...
      groups.forEach(function(g) {
        ...
        var c = cfn(v);
        h += '<td style="text-align:center;padding:6px 3px;border-radius:4px;font-weight:500;font-size:10px;background:' + c.b + ';color:' + c.f + '" title="' + ml + ' × ' + g + ': ' + c.t + '">' + c.t + '</td>';
      });
      h += '</tr>';
    });
    h += '</tbody></table>';
